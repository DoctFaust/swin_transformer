...
class SegBaseModel(nn.Module):
    ...
    def __init__(self, nclass, aux, backbone='resnet50', jpu=False, pretrained_base=True, **kwargs):
        ...
        if backbone == 'resnet50':
            ...
            # 获取原始第一个卷积层的信息
            original_conv1 = self.pretrained.conv1
            # 创建新的卷积层，输入通道改为4
            new_conv1 = nn.Conv2d(
                in_channels=10,
            ...
            # 如果加载预训练权重，初始化新卷积层的参数
            if pretrained_base:
                # 原始权重形状 [64, 3, 7, 7]
                original_weight = original_conv1.weight.data
                # 计算前三通道的均值作为第四个通道的初始化
                mean_weight = original_weight.mean(dim=1, keepdim=True)
                mean_weight = mean_weight.repeat(1, 7, 1, 1)  # 重复该均值生成 3 个新的通道
                # 拼接新权重 [64, 4, 7, 7]
                new_weight = torch.cat([original_weight, mean_weight], dim=1)
                new_conv1.weight.data = new_weight

            # 替换网络中的第一个卷积层
            self.pretrained.conv1 = new_conv1
