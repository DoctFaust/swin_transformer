...
class EGA(nn.Module):
    ...
    def forward(self, edge_feature, x, pred):
        ...
        if pred.size(1) == 2:  # 二分类 logits
            pred = torch.softmax(pred, dim=1)[:, 1:2]  # 取类别1的概率
        else:
            pred = torch.sigmoid(pred)  # 单通道 logits
        ...
        if edge_feature is None:
            ...
            # 确保分辨率匹配
            edge_feature = F.interpolate(edge_feature, size=xsize, mode='bilinear', align_corners=True)

        # high-frequency feature
        # edge_input = F.interpolate(edge_feature, size=xsize, mode='bilinear', align_corners=True)
        edge_input = F.interpolate(edge_feature, x.shape[2:], mode='bilinear', align_corners=True)
