...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default='my_test_data', choices=['potsdam', 'vaihingen'])
    parser.add_argument("--val_batchsize", type=int, default=2)
    parser.add_argument("--crop_size", type=int, nargs='+', default=[128, 128], help='H, W')
    parser.add_argument("--models", type=str, default='swinT',
...
def mutil_scale_val(model, weight_path, object_path):
    ...
    with torch.no_grad():
        ...
        if os.path.isfile(model_state_file):
            ...
            checkpoint = {k.replace('model.', 'module.'): v for k, v in checkpoint.items()}
            # 新增：过滤掉 ce_loss 相关参数
            checkpoint = {k: v for k, v in checkpoint.items() if not k.startswith('ce_loss.')}
            model.load_state_dict(checkpoint, strict=False)  # 允许部分加载
        else:
            warnings.warn('weight is not existed !!!"')
